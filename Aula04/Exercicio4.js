...
// Gere uma lista contendo o nome de todos os cantores, sem
// repetições
const semRepeticao = albuns.map (function(valor) {
    let cantoresSemRepeticao = [];
    if (cantoresSemRepeticao.indexOf(valor.cantor) === -1) {
        cantoresSemRepeticao.push(valor.cantor)
    }
    return cantoresSemRepeticao;
})
